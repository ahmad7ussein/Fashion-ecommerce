...
import {
  partnerPanelApi,
  type PartnerPanelAnalytics,
  type PartnerPanelProduct,
  type PartnerPanelStore,
} from "@/lib/api/partnerPanel"
...
const defaultFormState: PartnerFormState = {
  name: "",
  description: "",
  price: "",
  image: "",
...
export default function PartnerPanelPage() {
  ...
  const [store, setStore] = useState<PartnerPanelStore | null>(null)
  const [storeError, setStoreError] = useState<string | null>(null)
  const [analytics, setAnalytics] = useState<PartnerPanelAnalytics | null>(null)
  const [products, setProducts] = useState<PartnerPanelProduct[]>([])
  const [formState, setFormState] = useState<PartnerFormState>(defaultFormState)
    ...
    roleAssignmentsApi
      ...
      .then((assignments) => {
        const isActive = assignments.some(
          (assignment) => assignment.role === "partner" && assignment.status === "active"
        )
  ...
  const handleEdit = (product: PartnerPanelProduct) => {
            ...
            <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  ...
                  placeholder="https://partner-store.com/product/123"
                />
              </div>
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(event) => setFormState({ ...formState, commissionRate: event.target.value })}
                />
              </div>
