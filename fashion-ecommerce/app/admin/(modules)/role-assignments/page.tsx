...
import { roleAssignmentsApi, type RoleAssignment } from "@/lib/api/roleAssignments"
import { partnersApi, type PartnerStore } from "@/lib/api/partners"
...
const emptyForm = {
  email: "",
  role: "service_provider" as "service_provider" | "partner",
  partnerStoreId: "",
  notes: "",
}
...
export default function RoleAssignmentsPage() {
  ...
  const toggleStatus = async (assignment: RoleAssignment) => {
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <select
              ...
              value={form.role}
              onChange={(event) => setForm({ ...form, role: event.target.value as "service_provider" | "partner" })}
            >
            ...
            {form.role === "partner" && (
              <select
                className="h-10 rounded-md border px-3"
                value={form.partnerStoreId}
                onChange={(event) => setForm({ ...form, partnerStoreId: event.target.value })}
              >
