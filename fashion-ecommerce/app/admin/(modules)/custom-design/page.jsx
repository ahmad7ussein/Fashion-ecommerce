...
import { customDesignRequestsApi } from "@/lib/api/customDesignRequests";
    ...
    return (<div className="p-6 space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={settings.allowText} onChange={(event) => setSettings({ ...settings, allowText: event.target.checked })}/>
            Allow text
          </label>
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={settings.allowImages} onChange={(event) => setSettings({ ...settings, allowImages: event.target.checked })}/>
            Allow images
          </label>
          <label className="flex items-center gap-2 text-sm">
            <input type="checkbox" checked={settings.requireApproval} onChange={(event) => setSettings({ ...settings, requireApproval: event.target.checked })}/>
            Require admin approval
          </label>
          <Input type="number" placeholder="Max text length" value={settings.maxTextLength} onChange={(event) => setSettings({ ...settings, maxTextLength: Number(event.target.value) })}/>
          <Textarea placeholder="Allowed fonts (comma separated)" value={settings.allowedFonts.join(", ")} onChange={(event) => setSettings({
            ...settings,
          ...
          <Textarea placeholder="Print areas JSON" value={printAreasText} onChange={(event) => setPrintAreasText(event.target.value)}/>
          <div className="grid gap-3 md:grid-cols-3">
            <Input type="number" placeholder="Extra price (text)" value={settings.additionalPrices.text} onChange={(event) => setSettings({
            ...settings,
            additionalPrices: { ...settings.additionalPrices, text: Number(event.target.value) },
        })}/>
            <Input type="number" placeholder="Extra price (image)" value={settings.additionalPrices.image} onChange={(event) => setSettings({
            ...settings,
            additionalPrices: { ...settings.additionalPrices, image: Number(event.target.value) },
        })}/>
            <Input type="number" placeholder="Extra price (size)" value={settings.additionalPrices.size} onChange={(event) => setSettings({
            ...settings,
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {requests.map((request) => (<TableRow key={request._id}>
