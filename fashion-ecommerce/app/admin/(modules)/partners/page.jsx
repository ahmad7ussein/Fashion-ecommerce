...
export default function PartnerStoresPage() {
    ...
    return (<div className="p-6 space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Input placeholder="Store name" value={storeForm.name} onChange={(event) => setStoreForm({ ...storeForm, name: event.target.value })}/>
            <Input placeholder="Slug (unique)" value={storeForm.slug} onChange={(event) => setStoreForm({ ...storeForm, slug: event.target.value })}/>
            <Input placeholder="Website" value={storeForm.website} onChange={(event) => setStoreForm({ ...storeForm, website: event.target.value })}/>
            <Input type="number" placeholder="Default commission rate (%)" value={storeForm.defaultCommissionRate} onChange={(event) => setStoreForm({ ...storeForm, defaultCommissionRate: Number(event.target.value) })}/>
            <Input placeholder="Contact email" value={storeForm.contactEmail} onChange={(event) => setStoreForm({ ...storeForm, contactEmail: event.target.value })}/>
            <Input placeholder="Contact phone" value={storeForm.contactPhone} onChange={(event) => setStoreForm({ ...storeForm, contactPhone: event.target.value })}/>
            <Textarea placeholder="Description" value={storeForm.description} onChange={(event) => setStoreForm({ ...storeForm, description: event.target.value })}/>
          </div>
          <div className="flex gap-2">
            <Button onClick={handleStoreSubmit}>{editingStoreId ? "Update Store" : "Create Store"}</Button>
            {editingStoreId && (<Button variant="outline" onClick={() => {
                setEditingStoreId(null);
                setStoreForm({ ...emptyStore });
            }}>
                Cancel
      ...
      <Card>
        ...
        <CardContent>
          <Table>
            ...
            <TableBody>
              {stores.map((store) => (<TableRow key={store._id}>
          ...
          {analytics && analyticsStoreId && (<div className="mt-4 text-sm">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-2">
            <select className="h-10 rounded-md border px-3" value={productForm.storeId} onChange={(event) => setProductForm({ ...productForm, storeId: event.target.value })}>
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                <input id="partner-image-upload" type="file" accept="image/*" className="sr-only" onChange={handleImageUpload}/>
                <label htmlFor="partner-image-upload" className="absolute right-2 top-1/2 -translate-y-1/2 cursor-pointer text-muted-foreground hover:text-foreground" title="Upload image">
            ...
            <Input placeholder="Category" value={productForm.category} onChange={(event) => setProductForm({ ...productForm, category: event.target.value })} list="partner-category-options"/>
            <Input placeholder="Gender" value={productForm.gender} onChange={(event) => setProductForm({ ...productForm, gender: event.target.value })} list="partner-gender-options"/>
            <Input placeholder="Season" value={productForm.season} onChange={(event) => setProductForm({ ...productForm, season: event.target.value })} list="partner-season-options"/>
            <Input placeholder="Style" value={productForm.style} onChange={(event) => setProductForm({ ...productForm, style: event.target.value })} list="partner-style-options"/>
            <Input placeholder="Occasion" value={productForm.occasion} onChange={(event) => setProductForm({ ...productForm, occasion: event.target.value })} list="partner-occasion-options"/>
            <Input placeholder="Sizes (comma separated)" value={productForm.sizes} onChange={(event) => setProductForm({ ...productForm, sizes: event.target.value })} list="partner-size-options"/>
            <Input placeholder="Colors (comma separated)" value={productForm.colors} onChange={(event) => setProductForm({ ...productForm, colors: event.target.value })} list="partner-color-options"/>
            ...
            <Input placeholder="Product URL" value={productForm.productUrl} onChange={(event) => setProductForm({ ...productForm, productUrl: event.target.value })}/>
            <Input type="number" placeholder="Commission rate (override)" value={productForm.commissionRate} onChange={(event) => setProductForm({ ...productForm, commissionRate: event.target.value })}/>
