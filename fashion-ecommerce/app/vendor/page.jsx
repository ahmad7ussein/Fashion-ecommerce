...
export default function VendorPanelPage() {
    ...
    useEffect(() => {
        ...
        roleAssignmentsApi
            .getMyAssignments()
            .then((assignments) => {
            const isActive = assignments.some((assignment) => assignment.role === "service_provider" && assignment.status === "active");
    ...
    return (<div className="min-h-screen bg-muted/20 p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (<p className="text-sm text-muted-foreground">Loading products...</p>) : products.length === 0 ? (<p className="text-sm text-muted-foreground">No vendor products yet.</p>) : (<Table>
                <TableHeader>
                  <TableRow>
        ...
        <Card>
          ...
          <CardContent>
            {loading ? (<p className="text-sm text-muted-foreground">Loading orders...</p>) : orders.length === 0 ? (<p className="text-sm text-muted-foreground">No orders found yet.</p>) : (<Table>
                <TableHeader>
                  <TableRow>
