      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12 xl:px-24 py-6 sm:py-8 md:py-12">
        <div className="max-w-6xl mx-auto">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sm:mb-12 gap-4">
            ...
            <Button variant="outline" onClick={handleLogout} className="h-11 sm:h-12 px-6 sm:px-8 bg-white border-2 border-gray-200 hover:bg-rose-50 hover:border-rose-300 text-gray-700 hover:text-rose-600 rounded-full text-sm sm:text-base font-medium">
              Logout
            </Button>
          </motion.div>
          ...
          <Tabs defaultValue="profile" className="space-y-6 sm:space-y-8">
            ...
            <TabsContent value="orders">
              {loadingOrders ? (<div className="text-center py-12">
                ...
                </div>) : orders.length === 0 ? (<Card className="bg-white backdrop-blur-sm border-2 border-gray-200 hover:border-rose-300 shadow-xl rounded-xl sm:rounded-2xl">
                  ...
                  {orders.map((order) => (<Card key={order._id} className="bg-white backdrop-blur-sm border-2 border-gray-200 hover:border-rose-300 hover:shadow-xl transition-all rounded-xl sm:rounded-2xl">
                    ...
                    : order.status === "shipped"
                        ? "secondary"
                        : "outline"} className="bg-rose-500 text-white">
                            {order.status}
                          </Badge>
                        </div>
              ...
              {loadingDesigns ? (<div className="text-center py-12">
                  ...
                  {allDesigns.map((design) => (<Card key={design._id} className="overflow-hidden bg-white backdrop-blur-sm border-2 border-gray-200 hover:border-rose-300 hover:shadow-xl transition-all rounded-xl sm:rounded-2xl">
                      ...
                      <CardContent className="p-4">
                        ...
                        <div className="flex gap-2">
                          <Button size="sm" className="flex-1 bg-gradient-to-r from-rose-500 to-pink-500 text-white hover:from-rose-600 hover:to-pink-600 rounded-full" asChild>
                            <Link href={getDesignLink(design)}>Edit Design</Link>
                          </Button>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <Input id="zipCode" value={shippingAddress.zipCode} onChange={(e) => setShippingAddress({ ...shippingAddress, zipCode: e.target.value })}/>
                    </div>
                    <div>
