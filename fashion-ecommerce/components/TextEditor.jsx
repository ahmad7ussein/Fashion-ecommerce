...
export const TextEditor = ({ text, onTextChange, fontSize, onFontSizeChange, textAlign, onTextAlignChange, fontFamily, }) => {
    return (<div className="space-y-4">
      <div className="space-y-2">
        ...
        <textarea value={text} onChange={(event) => onTextChange(event.target.value)} placeholder="اكتب النص هنا..." className="w-full h-24 px-4 py-3 rounded-lg border border-border bg-background text-foreground placeholder:text-muted-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary transition-all" dir="rtl" style={{ fontFamily }}/>
      </div>

      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <button onClick={() => onFontSizeChange(Math.max(12, fontSize - 2))} className="w-8 h-8 rounded border border-border flex items-center justify-center hover:bg-muted transition-colors">
            <Minus className="w-3 h-3"/>
          </button>
          <span className="text-sm font-medium min-w-[40px] text-center">
            A<span className="text-xs">A</span>A
          </span>
          <button onClick={() => onFontSizeChange(Math.min(48, fontSize + 2))} className="w-8 h-8 rounded border border-border flex items-center justify-center hover:bg-muted transition-colors">
            <Plus className="w-3 h-3"/>
          </button>
        </div>

        <div className="flex items-center gap-1 border border-border rounded-lg p-1">
          <button onClick={() => onTextAlignChange("right")} className={cn("w-8 h-8 rounded flex items-center justify-center transition-colors", textAlign === "right" ? "bg-muted" : "hover:bg-muted/50")}>
            <AlignRight className="w-4 h-4"/>
          </button>
          <button onClick={() => onTextAlignChange("center")} className={cn("w-8 h-8 rounded flex items-center justify-center transition-colors", textAlign === "center" ? "bg-muted" : "hover:bg-muted/50")}>
            <AlignCenter className="w-4 h-4"/>
          </button>
          <button onClick={() => onTextAlignChange("left")} className={cn("w-8 h-8 rounded flex items-center justify-center transition-colors", textAlign === "left" ? "bg-muted" : "hover:bg-muted/50")}>
    ...
    return (<div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
      ...
      <div className="space-y-2 min-w-0">
        ...
        <Select value={textColor} onValueChange={onTextColorChange}>
          ...
          <SelectContent className="text-right">
            {textColors.map((color) => (<SelectItem key={color.value} value={color.value}>
                <span className="flex items-center gap-2">
                  <span className={cn("h-4 w-4 rounded-full border", color.value === "#ffffff" && "border-border")} style={{ backgroundColor: color.value }}/>
