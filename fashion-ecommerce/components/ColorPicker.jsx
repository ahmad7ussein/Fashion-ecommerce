...
export const ColorPicker = ({ selectedColor, onColorChange, colors }) => {
    ...
    return (<div className="space-y-3">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {options.map((color) => (<button key={color.id} onClick={() => onColorChange(color.value)} className={cn("w-9 h-9 rounded-full transition-all duration-200 hover:scale-110", color.needsBorder && "border border-border", selectedValue === color.value.toLowerCase() && "ring-2 ring-primary ring-offset-2 ring-offset-background scale-110")} style={{ backgroundColor: color.value }} title={color.label} aria-label={color.label}/>))}
      </div>
    </div>);
};
