...
export function OrderTracker({ points, width = 640, height = 280, durationSeconds = 12, }) {
    ...
    return (<div className="w-full flex justify-center">
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%" style={{ maxWidth: width }} className="overflow-visible">
        <defs>
        ...
        <motion.path ref={pathRef} d={pathD} fill="none" stroke="url(#trackGradient)" strokeWidth={6} strokeLinecap="round" strokeLinejoin="round" filter="url(#glow)" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 1.2, ease: "easeInOut" }}/>

        
        {points.map((p, idx) => {
            const isPassed = progress.get() >= segmentRatios[idx] - 0.001;
            return (<g key={`${p.x}-${p.y}-${idx}`}>
              <circle cx={p.x} cy={p.y} r={10} fill={isPassed ? "#22d3ee" : "#cbd5e1"} stroke="#0ea5e9" strokeWidth={2}/>
              {p.label ? (<text x={p.x + 14} y={p.y - 14} fontSize={12} fontWeight={600} fill="#0f172a" style={{ pointerEvents: "none" }}>
                  {p.label}
                </text>) : null}
            </g>);
        })}
        ...
        <motion.g initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} transform={`translate(${carPos.x - 14}, ${carPos.y - 10})`}>
