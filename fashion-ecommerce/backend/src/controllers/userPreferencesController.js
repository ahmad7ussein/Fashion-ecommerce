...
const getUserPreferences = async (req, res) => {
    try {
        ...
        let preferences;
        try {
            preferences = await UserPreferences_1.default.findOne({ user: userId });
        }
        catch (dbError) {
...
const updateUserPreferences = async (req, res) => {
    try {
        ...
        if (!cleanedUpdateData || (typeof cleanedUpdateData === 'object' && Object.keys(cleanedUpdateData).length === 0)) {
            try {
                const currentPreferences = await UserPreferences_1.default.findOne({ user: userId });
                ...
                const defaultPreferences = await UserPreferences_1.default.create({
                    user: userId,
                });
                return res.status(200).json({
                    success: true,
